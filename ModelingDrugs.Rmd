...
new_drugsdf <- new_df |>
  ...
  select(x5_digit_fips_code, name, state_abbreviation, Drug_Overdose_Deaths, Unemployment_Rate, Income_Inequality, suicides, mental_ratio, Adult_Smoking_Rate, Median_House_Income, Highschool_Comp_Rate, Single_Parent_House, Social_Associations,
         NH_Black, American_Or_Alaska_Native, Hispanic_Pop, Female_Pop, region, pop_den, urban_pop, urban_level,
         Urban_yes, urban_num, urban_num_twolev) |>
...
filtered_tidy_high <- tidy_model %>%
  ...
  arrange(desc(level)) |>
  select(level, estimate) # states that have baseline high intercepts in desc order
...
States in midwest and east coast has highest baseline drug overdose deaths. Thus, we should focus drug overdose prevention efforts on these states.  